...
//	REG_EXMEMCNT  = (REG_EXMEMCNT & ~0x001f) | 0x001a; // ROM cycle=4,6 , RAM cycle = 6
...
#if DEBUG_MSG
	_consolePrintf("Wait for DONE=L\n");
#endif
...
#if DEBUG_MSG
	_consolePrintf("Wait for INIT=H\n");
#endif
	...
	for(i=0;i<config_data_size;i++)
	{
		...
		*FPGA_CFG_DATA = wdata;
		...
		{
#if DEBUG_MSG
			_consolePrintf("Configration Data Error\n");
#endif
			REG_IME = IME_ENABLE;
...
#if DEBUG_MSG
	_consolePrintf("Wait for DONE=H\n");
#endif
	...
	_consolePrintf("Configration Done\n");
	REG_IME = IME_ENABLE;
...
_consolePrintf("Supported REV2 only. halt.\n");
//	ShowLogHalt();
	while(1);
}
...
{
	_consolePrintf("NDS FPGA MDX play tester\n");
