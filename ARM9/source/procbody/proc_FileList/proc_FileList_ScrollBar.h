...
static void ScrollBar_Init(TScrollBar *psb,s32 _ItemHeight)
{
  psb->LastMouseX=0;
  psb->LastMouseY=0;
  ...
  psb->pbm=new CglCanvas(NULL,psb->DrawRect.w,psb->DrawRect.h,pf15bit);
  psb->TopPos=0;
  psb->ShowPos=0;
  psb->MaxPos=0;
  psb->LastPos=0;
...
static void ScrollBar_Init(TScrollBar *psb,s32 _ItemHeight)
{
  ...
  psb->SelectedIndex=0;
  psb->ItemHeight=_ItemHeight;
  psb->ClientSize=ScreenHeight-16*3;
  psb->DrawRect=CreateRect(ScreenWidth-14,32,10,psb->ClientSize-12);
  psb->ClickMarginWidth=24;
  psb->pbm=new CglCanvas(NULL,psb->DrawRect.w,psb->DrawRect.h,pf15bit);
  psb->TopPos=0;
  psb->ShowPos=0;
  ...
  psb->ButtonWidth=10;
  psb->ButtonHeight=6;
  psb->GripTop=psb->ButtonHeight+32;
  psb->GripHeight=psb->DrawRect.h-(psb->ButtonHeight);
  psb->GripAlignY=0;
  psb->CurMode=ESBCM_Idle;
  psb->UsePressImage=false;
}
...
static bool ScrollBar_MouseDown(TScrollBar *psb,s32 x,s32 y)
{
  ...
  if(psb->CurMode!=ESBCM_Idle){
    _consolePrintf("FatalError: ScrollBar_MouseDown psb->CurMode(%d)!=ESBCM_Idle\n",psb->CurMode);
    ShowLogHalt();
  }
...
asm void ScrollBar_Draw_asm_DrawBG_Fixed75per(u16 *pbuf)
{
  push {r4,r5,r6,r7,lr}
...
//  DrawSkinAlpha(ScrollBarAlpha_GetSkin(ESBSA_BG),ptagbm,dx,dy);
  ...
  if(psb->MaxPos==0){
    _consolePrintf("FatalError: ScrollBar_MouseIdle psb->MaxPos==0\n");
    ShowLogHalt();
  }
...
//  DrawSkinAlpha(ScrollBarAlpha_GetSkin(ESBSA_BG),ptagbm,dx,dy);
 // ScrollBar_Draw_asm_DrawBG_Fixed75per(ptagbm->GetVRAMBuf());
  ...
  if(psb->UsePressImage==false){
    pUpBtn=ScrollBarAlpha_GetSkin(EFLSA_scroll_block);
    pDownBtn=ScrollBarAlpha_GetSkin(EFLSA_scroll_block);
    switch(psb->CurMode){
      case ESBCM_Idle: break;
      case ESBCM_PressUpButton: {
        pUpBtn=ScrollBarAlpha_GetSkin(EFLSA_scroll_block);
...
/*  if(psb->MaxPos==0){
  ...
  CglTGF *psb_BG,*pGripM3,*pGripBG,*pGripTop,*pGripBottom;
  psb_BG=ScrollBarAlpha_GetSkin(EFLSA_scroll_bar);
  psb_BG->BitBlt(ptagbm,dx,dy);
  
  //pGripM3=ScrollBarAlpha_GetSkin(EFLSA_scroll_block);
  ...
  bary+=pGripTop->GetHeight();
  barh-=pGripTop->GetHeight();
 // DrawSkinAlpha(pGripM3,ptagbm,dx,bary+barh-pGripBottom->GetHeight()/2-4);
  DrawSkinAlpha(pGripBottom,ptagbm,dx,bary+barh-pGripBottom->GetHeight());
  barh-=pGripBottom->GetHeight();
