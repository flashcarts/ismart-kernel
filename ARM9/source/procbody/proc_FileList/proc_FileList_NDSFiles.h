...
enum ENDSFile_FileType {ENFFT_UnknownFile,ENFFT_UpFolder,ENFFT_Folder,ENFFT_Sound,ENFFT_Image,ENFFT_Text,ENFFT_Video,ENFFT_NDSROM,ENFFT_Skin,ENFFT_GBA,};
...
typedef struct {
    ...
    u32 Ext32;
    u32 FileSize;
    TFAT2_TIME FileLastWriteDateTime;
...
static bool isHiddenExt32(u32 Ext32)
{
  ...
  if(Ext32==MakeExt32(0,'M','P','3')) return(true);
  if(Ext32==MakeExt32(0,'J','P','G')) return(true);
  extern bool isOnlyNDS;
  if(isOnlyNDS)
  {
  ...
  {  
	  ...
	  if(Ext32==MakeExt32(0,'S','A','V')) return(false);
	  if(Ext32==MakeExt32(0,'D','A','T')) return(false);
	  if(ExtLink_GetTargetIndex(Ext32)!=(u32)-1) return(false);
  }
  
  if(Ext32==MakeExt32(0,'D','P','G')) return(true);
  
  if(Ext32==MakeExt32(0,'S','K','N')) return(true);
...
static void NDSFiles_RefreshCurrentFolder(void)
{
            ...
            if((FAT2_GetAttrib()&ATTRIB_HID)==0)
            {
                ...
                {          	
                    ...
                    }else
                    {
                        if(strcmp(pafn,".")!=0)
                        {
                        	if((strcmp(pafn,"ROMDAT")!=0) && (strcmp(pafn,"system")!=0))
                        		NDSFilesCount++;
                        	}
                    }            	               	
                } break;
    ...
    for(s32 idx=0;idx<NDSFilesCount;idx++)
    {
        ...
        pfile->Ext32=0;
        pfile->FileSize=0;
        //pfile->pFileLastWriteDateTime=NULL;
        pfile->pFilenameAlias=NULL;
        pfile->pFilenameUnicode=NULL;
        pfile->pFilenameUnicode_DoubleLine0=NULL;
        pfile->pFilenameUnicode_DoubleLine1=NULL;
  ...
  
  {
    ...
    while(FAT_FileType!=FT_NONE){
      if((FAT2_GetAttrib()&ATTRIB_HID)==0){
        ...
        FAT2_GetAttrib();
        TFAT2_TIME lwdt=FAT2_GetFileLastWriteTime();
        switch(FAT_FileType){
          case FT_NONE: break;
          case FT_DIR: {
          		if((strcmp(pafn,"..")==0)){
	              ...
	              pfile->Ext32=0;
	              pfile->FileLastWriteDateTime=lwdt;
          ...
          case FT_FILE: {
        	  if(strcmp(pafn,"SYS.BIN"))
        	  {            	  
              	FAT_FILE* FAT_FileOpen;
           	    FAT_FileOpen = FAT2_fopen_AliasForRead(pafn);
           	    u32 FAT_FileSize=  FAT2_GetFileSize(FAT_FileOpen);
           	    //TFAT2_TIME lwdt=FAT2_GetFileLastWriteTime();
           	    FAT2_fclose(FAT_FileOpen);
                  u32 Ext32=0;
                  {
                    const char *ptmp=pafn;
                    while(*ptmp!=0){
...
//    _consolePrintf("%d/%d %s",idx,NDSFilesCount,pfile->pFilenameAlias);
    ...
    if(pfile->FileType==ENFFT_UnknownFile){
      if((pfile->Ext32==MakeExt32(0,'N','D','S')) || (pfile->Ext32==MakeExt32(0,'I','D','S'))) pfile->FileType=ENFFT_NDSROM;
    }
    
    if(ExtLink_GetTargetIndex(pfile->Ext32)!=(u32)-1) pfile->FileType=ENFFT_NDSROM;
    
    if(pfile->FileType==ENFFT_UnknownFile){
         if(pfile->Ext32==MakeExt32(0,'G','B','A')) pfile->FileType=ENFFT_GBA;
       }
    ...
    switch(pfile->FileType){
      ...
      case ENFFT_Sound: pfile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_UnknownFile); break;
      case ENFFT_Image: pfile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_UnknownFile); break;
      case ENFFT_Text: pfile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_UnknownFile); break;
      case ENFFT_Video: pfile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_UnknownFile); break;
      case ENFFT_GBA: pfile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_GBA); break;
      case ENFFT_NDSROM: 
          {
      ...
      case ENFFT_Skin: pfile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_UnknownFile); break;
      default: {
...
static void NDSFiles_LoadNDSIcon(TNDSFile *pNDSFile)
{
  ...
  if((pNDSFile->Ext32==MakeExt32(0,'N','D','S'))|| (pNDSFile->Ext32==MakeExt32(0,'I','D','S'))){
    if(NDSROMIcon_Get(pNDSFile->pFilenameAlias,pNDSFile->pNDSROMIcon,pNDSFile->pJpnTitle,pNDSFile->pEngTitle)==false){
  ...
  if(NDSROMIcon_Get(pAlias,pNDSFile->pNDSROMIcon,pNDSFile->pJpnTitle,pNDSFile->pEngTitle)==false){
    safefree(pNDSFile->pNDSROMIcon); pNDSFile->pNDSROMIcon=NULL;
    pNDSFile->pIcon=FileListAlpha_GetSkin(EFLSA_Icon_NDSROM);
  }
}
