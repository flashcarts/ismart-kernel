...
const TglUnicode *CglCanvas::TextOutW_WidthOther(const int x,const int y,const int widthother,const TglUnicode *str) const
{
  int dx=x;
  int dy=y;
...
void CglCanvas::TextOutUTF8_StartMid(const int x,const int y,const int wbutton,const char *str) const
{
  if(str==NULL) return;
  if(*str==0) return;
  char strtmp[256];
  MemSet8CPU(0x00,strtmp,256);
  MemCopy8CPU(str,strtmp,256);
  int dx=x;
  int dy=y;
