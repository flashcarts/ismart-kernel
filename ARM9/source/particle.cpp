...
static bool Particle_MouseDown;
static u32 Particle_MouseX,Particle_MouseY;
...
void Particle_Init(void)
{
  ...
  pParticle=(TParticle*)safemalloc(sizeof(TParticle));
  if(pParticle==NULL){
    _consolePrintf("Fatal error: Particle_Init: Memory overflow.\n");
    ShowLogHalt();
  }
...
void Particle_Free(void)
{
  if(pParticle->pdstbuf!=NULL){
    safefree(pParticle->pdstbuf); pParticle->pdstbuf=NULL;
  }
  
  if(pParticle!=NULL){
    safefree(pParticle); pParticle=NULL;
  }
}
...
void Particle_Update(u32 VsyncCount,CglCanvas *pcan)
{
  ...
  if(pParticle->pdstbuf==NULL){
    firststart=true;
    pParticle->pdstbuf=(u8*)safemalloc(ScreenWidth*ScreenHeight);
    }else{
    firststart=false;
  }
  ...
  
  {
    ...
    if(touch==true){
      ...
      if(firststart==true){
        ...
        for(u32 idx=1;idx<PartsCount;idx++){ // idx0は処理しない
          TParticle_Part *ppart=&pParticle->Parts[idx];
          ppart->x=ptagpart->x;
          ppart->y=ptagpart->y;
        }
