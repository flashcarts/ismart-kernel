...
FAT_FILE* Shell_FAT_fopen_SJISToUnicodeTable(void)
{
  ...
  {
	  const char *pfullalias;
	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath FontFrench "/sjis.tbl");
	  _consolePrintf("Shell_FAT_fopen_Data=%s\n",pfullalias);
	  return(FAT2_fopen_AliasForRead(pfullalias));
  }
}
...
FAT_FILE* Shell_FAT_fopen_LocalToUnicodeTable(void)
{
  const char *pfullalias;
  if(GetKoreanFlg())
  {
	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath FontKorean "/system.l2u");	    
  }
  else if(GetFrenchFlg())
  {
	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath FontFrench "/system.l2u");
  }
  else
  {
	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath "/system.l2u");	      
  }
  _consolePrintf("Shell_FAT_fopen_Data=%s\n",pfullalias);
  return(FAT2_fopen_AliasForRead(pfullalias));
}
FAT_FILE* Shell_FAT_fopen_AnkToUnicodeTable(void)
{
  const char *pfullalias;
  if(GetKoreanFlg())
   {
 	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath FontKorean "/system.ank");	    
   }
  else if(GetFrenchFlg())
  {
	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath FontFrench "/system.l2u");
   ...
   {
 	  pfullalias=ConvertFullPath_Ansi2Alias(DefaultDataPath "/system.ank");	      
   }
  _consolePrintf("Shell_FAT_fopen_Data=%s\n",pfullalias);
  return(FAT2_fopen_AliasForRead(pfullalias));
}
...
FAT_FILE* Shell_FAT_fopen_Language_chrglyph(void)
{
    ...
    {
    	snprintf(fullfn,256,DefaultDataPath "/" FontLibName);  
    }
    //if(CodePageStr[0]=='0'){
...
/*	    if(MulLangDataCount==0)
	    {
	    	CurProcState->SetupW.WhichL=0;
	    	snprintf(fullfn,256,DefaultDataPath "/system.msg");
*
	    }*/
	    //else 
	    {
	    	if((CurProcState->SetupW.WhichL > MulLangListFileNum-1) || (CurProcState->SetupW.WhichL < 0))
	    	{
	    		CurProcState->SetupW.WhichL=0;
	    		snprintf(fullfn,256,DefaultDataPath "/language/%s",MulLangListFile[CurProcState->SetupW.WhichL].Filename);
	    	}
	    	else
	    	{
	    		//UnicodeChar dststr[256];
	    		//StrConvert_UTF82Unicode(MulLangListFile[CurProcState->SetupW.WhichL-1].Filename,dststr);
	    		snprintf(fullfn,256,DefaultDataPath "/language/%s", MulLangListFile[CurProcState->SetupW.WhichL].Filename);
	    	}	    	
	    }
	    
	  //  videoSub_SetShowLog(true);
	    _consolePrintf("\n cmp1=%s",MulLangListFile[CurProcState->SetupW.WhichL].Filename);
	   // TestPauseKey();
...
FAT_FILE* Shell_FAT_fopen_Mul_Language_messages(void)
{
	static char fullfn[256];
   
    snprintf(fullfn,256,DefaultDataPath "/language/Mul_language.ini");  
    const char *pfullalias;
    pfullalias=ConvertFullPath_Ansi2Alias(fullfn);
    _consolePrintf("Shell_FAT_fopen_Data=%s\n",pfullalias);
    return(FAT2_fopen_AliasForRead(pfullalias));    
...
FAT_FILE* Shell_FAT_fopen_Mul_Skin(void)
{
	static char fullfn[256];
   
    snprintf(fullfn,256,DefaultDataPath "/" SkinLstName);  
    const char *pfullalias;
    pfullalias=ConvertFullPath_Ansi2Alias(fullfn);
    _consolePrintf("Shell_FAT_fopen_Data=%s\n",pfullalias);
    return(FAT2_fopen_AliasForRead(pfullalias));    
...
void Shell_CheckDataPath(void)
{
  ...
  if(pf==NULL){
    _consolePrintf("Can not found data files. (" DefaultDataPath "/" DataFilename ")\n");
    ShowLogHalt();
  }
