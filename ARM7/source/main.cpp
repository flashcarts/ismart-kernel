...
void 	JmpGBA();
//void 	CopyAndBoot();
void 	bxr0(uint32 jmp,void*source,void*dest,uint32 size);
//void 	GetAndjumptoBoot(void);
#ifdef __cplusplus
}
#endif
  ...
  while (1){
	  /*****尼強gba*****/
	  	if(*(uint32*)0x2000000 == 0xFDEf54C8)
		{
			...
			uint16 tt;
			uint32 kk;
			 *(vuint16*)0x9000020 = 0xD978 ;
			o = *(uint16*)0x2000004 ;
			a = *(uint16*)0x2000006 ;
			
			//resetMemory_ARM7(false);
			
			   (*(vuint16*)0x04000304)  = 0x0;
			(*(vuint16*)0x4000204)= 0xF095 ;
				for(kk=0;kk<16;kk++)
					tt =*((vu16 *)(0x08400000+kk*2));
				SetRompage(o);
				SetRampage(a);
				for(kk=0;kk<16;kk++)
					tt =*((vu16 *)(0x8000000+kk*2));
				Switch2Sytem();
